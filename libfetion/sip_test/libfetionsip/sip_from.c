...
/*
 *  the from of SIP-C only have one field uid
 */

int
osip_message_set_from_c (osip_message_t * sip, const char *hvalue)
{
  int i;
  ...
  if (sip->from != NULL)
    return OSIP_SYNTAXERROR;
  i = osip_from_init_c (&(sip->from));
  if (i != 0)
    return i;
  sip->message_property = 2;
  i = osip_from_parse_c (sip->from, hvalue);
  if (i != 0)
    {
      osip_from_free (sip->from);
      sip->from = NULL;
...
osip_from_c_t *
osip_message_get_from_c (const osip_message_t * sip)
{
  return sip->from;
}
#endif
...
int
osip_from_init_c (osip_from_c_t ** from)
{
  *from = (osip_from_c_t *) osip_malloc (sizeof (osip_from_c_t));
  if (*from == NULL)
    return OSIP_NOMEM;
  (*from)->uid = NULL;
...
void
osip_from_free_c (osip_from_c_t * from)
{
  if (from == NULL)
    return;
...
int
osip_from_parse_c (osip_from_c_t * from, const char *hvalue)
{
  if( from == NULL || hvalue == NULL || hvalue[0] == '\0' )
    return OSIP_BADPARAMETER;
  size_t n_len = strlen( hvalue );
  from->uid = (char*)osip_malloc( n_len + 1 );
  memset( from->uid, 0, n_len + 1 );
  osip_strncpy( from->uid, hvalue, n_len );
  return OSIP_SUCCESS;
}

int
osip_from_to_str_c (const osip_from_c_t * from, char **dest)
{
  char *buf;
  size_t len = 0;

  *dest = NULL;
  if ((from == NULL) || (from->uid == NULL))
    return OSIP_BADPARAMETER;
  len = strlen( from->uid );
  buf = (char*)osip_malloc( len + 1 );
  osip_strncpy( buf, from->uid, len);
  *dest = buf;
  return OSIP_SUCCESS;
}
